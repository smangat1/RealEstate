...
import { attestCatalogSourceExact } from "@/lib/catalog-listing-sources";
...
export async function POST(
  ...
  try {
    ...
    await attestCatalogSourceExact({
    ...
    const message = error instanceof Error ? error.message : "Unable to confirm the listing source.";
    return NextResponse.json(
      { error: message === "MOBILE_AUTH_REQUIRED" ? "Unauthorized" : "Unable to confirm the listing source." },
