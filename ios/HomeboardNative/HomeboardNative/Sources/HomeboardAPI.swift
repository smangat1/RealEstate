...
private struct AdvisorContributionRequest: Encodable {
  var action: String
  var paymentMethod: String
  var transactionId: String?
}

private struct AdvisorSubscriptionResponse: Decodable {
  var subscription: ScoutSubscription
}

private struct AdvisorActionUpdateRequest: Encodable {
  var actionId: String
  var status: String
}

private struct AdvisorActionListResponse: Decodable {
  var actions: [AdvisorAction]
}

struct ListingCheckResponse: Decodable {
  var snapshotId: String
  var changes: [ListingChangeEntry]
  var actions: [AdvisorAction]
  var message: String
}

private struct ListingHistoryResponse: Decodable {
  var changes: [ListingChangeEntry]
}

private struct ApplicationChecklistResponse: Decodable {
  var items: [ApplicationChecklistEntry]
}

private struct ApplicationChecklistUpdateResponse: Decodable {
  var item: ApplicationChecklistEntry
}

private struct ApplicationChecklistUpdateRequest: Encodable {
  var itemId: String
  var status: String
}

private struct InquiryListResponse: Decodable {
  var inquiries: [ListingInquiry]
}

private struct InquiryCreateResponse: Decodable {
  var inquiry: ListingInquiry
  var action: AdvisorAction
}

private struct InquiryUpdateResponse: Decodable {
  var inquiry: ListingInquiry
}

private struct InquiryCreateRequest: Encodable {
  var templateKey: String
}

private struct InquiryUpdateRequest: Encodable {
  ...
  var reviewConfirmed: Bool?
}

private struct TourNoteRequest: Encodable {
  var transcript: String
}

private struct TourNoteResponse: Decodable {
  var id: String
  var summary: TourNoteSummary
  var action: AdvisorAction
}
...
final class HomeboardAPI {
  ...
  func startAdvisorSplit(accessToken: String, boardId: String) async throws -> ScoutSubscription {
    let response: AdvisorSubscriptionResponse = try await requestBackend(
      path: "/api/mobile/boards/\(boardId)/subscription",
      method: "POST",
      accessToken: accessToken,
      body: EmptyRequestBody()
    )
  ...
  func fundAdvisor(
    accessToken: String,
    boardId: String,
    coverRemaining: Bool = false
  ) async throws -> ScoutSubscription {
    let response: AdvisorSubscriptionResponse = try await requestBackend(
      path: "/api/mobile/boards/\(boardId)/subscription/contribute",
      method: "POST",
      accessToken: accessToken,
      body: AdvisorContributionRequest(
        action: coverRemaining ? "cover" : "contribute",
  ...
  func updateAdvisorAction(
    ...
    let _: EmptyResponse = try await requestBackend(
      path: "/api/mobile/boards/\(boardId)/advisor/actions",
      method: "PATCH",
      accessToken: accessToken,
      body: AdvisorActionUpdateRequest(actionId: actionId, status: status)
    )
  }

  func loadAdvisorActions(accessToken: String, boardId: String) async throws -> [AdvisorAction] {
    let response: AdvisorActionListResponse = try await requestBackend(
      path: "/api/mobile/boards/\(boardId)/advisor/actions",
      accessToken: accessToken
    )
  ...
  func checkListing(accessToken: String, boardId: String, listingId: String) async throws -> ListingCheckResponse {
    try await requestBackend(
      path: "/api/mobile/boards/\(boardId)/listings/\(listingId)/advisor/check",
      method: "POST",
      accessToken: accessToken,
      body: EmptyRequestBody()
    )
  ...
  func loadListingHistory(accessToken: String, boardId: String, listingId: String) async throws -> [ListingChangeEntry] {
    let response: ListingHistoryResponse = try await requestBackend(
      path: "/api/mobile/boards/\(boardId)/listings/\(listingId)/advisor/history",
      accessToken: accessToken
    )
    return response.changes
  }

  func loadApplicationChecklist(accessToken: String, boardId: String, listingId: String) async throws -> [ApplicationChecklistEntry] {
    let response: ApplicationChecklistResponse = try await requestBackend(
      path: "/api/mobile/boards/\(boardId)/listings/\(listingId)/advisor/application",
      accessToken: accessToken
    )
    return response.items
  }

  func updateApplicationChecklist(
    accessToken: String,
    boardId: String,
    listingId: String,
    itemId: String,
    status: String
  ) async throws -> ApplicationChecklistEntry {
    let response: ApplicationChecklistUpdateResponse = try await requestBackend(
      path: "/api/mobile/boards/\(boardId)/listings/\(listingId)/advisor/application",
      method: "PATCH",
      accessToken: accessToken,
      body: ApplicationChecklistUpdateRequest(itemId: itemId, status: status)
    )
    return response.item
  }

  func loadListingInquiries(accessToken: String, boardId: String, listingId: String) async throws -> [ListingInquiry] {
    let response: InquiryListResponse = try await requestBackend(
  ...
  func createInquiryDraft(
    accessToken: String,
    boardId: String,
    listingId: String,
    templateKey: String
  ) async throws -> ListingInquiry {
    let response: InquiryCreateResponse = try await requestBackend(
      path: "/api/mobile/boards/\(boardId)/listings/\(listingId)/outreach",
      method: "POST",
      accessToken: accessToken,
      body: InquiryCreateRequest(templateKey: templateKey)
    )
  ...
  func updateInquiry(
    ...
    let response: InquiryUpdateResponse = try await requestBackend(
      path: "/api/mobile/boards/\(boardId)/listings/\(listingId)/outreach",
      method: "PATCH",
      accessToken: accessToken,
  ...
  func saveTourNote(
    ...
    listingId: String,
    transcript: String
  ) async throws -> TourNoteSummary {
    let response: TourNoteResponse = try await requestBackend(
      path: "/api/mobile/boards/\(boardId)/listings/\(listingId)/advisor/tour-notes",
      method: "POST",
      accessToken: accessToken,
      body: TourNoteRequest(transcript: transcript)
    )
